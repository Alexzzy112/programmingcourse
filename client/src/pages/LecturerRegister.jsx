...
export default function LecturerRegister() {
  const [form, setForm] = useState({ firstName: '', lastName: '', email: '', password: '', confirmPassword: '', staffId: '', phone: '', department: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/auth/lecturer/register', form);
    ...
    <div className="min-h-screen bg-gradient-to-br from-primary-600 to-primary-800 flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          ...
          <p className="text-primary-200 mt-2">Lecturer Registration</p>
        </div>
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" name="staffId" className="input-field" value={form.staffId} onChange={handleChange} required />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Already have an account? <Link to="/lecturer/login" className="text-primary-600 hover:underline">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
